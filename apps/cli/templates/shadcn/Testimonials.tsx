...
function Avatar({ item }: { item: TestimonialItem }) {
  if (item.avatarUrl) {
    return <img src={item.avatarUrl} alt={item.author} className="h-10 w-10 rounded-full object-cover" />;
  }
  return (
    <div
        ...
        <section className={`px-6 py-24 ${altBg} sm:py-32`}>
          <div className="mx-auto max-w-6xl">
            ...
            <div className="flex snap-x snap-mandatory gap-6 overflow-x-auto pb-4">
              {items.map((item, i) => (
                <Card key={i} className="min-w-[320px] max-w-sm shrink-0 snap-start p-8 shadow-sm">
                ...
                <li key={i}>
                  <Card className="flex h-full flex-col gap-4 p-8 shadow-sm">
                    <CardContent className="flex flex-1 flex-col gap-4 p-0">
                      ...
                      <div className="flex items-center gap-3 border-t pt-4" style={{ borderColor: dividerColor }}>
                        <Avatar item={item} />
                        <div>
                          <p className={`text-sm font-semibold ${textPrimary}`}>{item.author}</p>
                          {item.role && <p className={`text-xs ${textMuted}`}>{item.role}</p>}
