        ...
        <section className={`px-6 py-24 ${altBg} sm:py-32`}>
          <div className="mx-auto max-w-4xl">
            ...
            <ul className="space-y-6">
              {items.map((item, i) => (
                <li key={i} className={`group flex gap-6 overflow-hidden rounded-2xl border ${cardBg} shadow-sm`}>
                  ...
                  <div className="flex flex-col justify-center py-4 pr-6">
                    ...
                    <div className="mt-3 flex items-center gap-4">
                      ...
                      <button
                        className="rounded-full px-5 py-1.5 text-sm font-semibold text-white transition-opacity hover:opacity-90"
                        style={{ backgroundColor: PRIMARY }}
                        onClick={e => e.preventDefault()}
                      >
                        Add to Cart
        ...
        <section className={`px-6 py-24 ${altBg} sm:py-32`}>
          <div className="mx-auto max-w-6xl">
            ...
            <div className="grid grid-cols-1 gap-8 lg:grid-cols-2">
              {first && (
                <div className={`group relative overflow-hidden rounded-3xl border ${cardBg} shadow-xl`}>
                  <div className="relative h-72 overflow-hidden">
                    <ProductImage item={first} i={0} />
                    {first.badge && <Badge label={first.badge} />}
                  </div>
                  <div className="p-8">
              ...
              <div className="grid grid-cols-1 gap-6 sm:grid-cols-2">
                {rest.slice(0, 4).map((item, i) => (
                  <div key={i} className={`group relative overflow-hidden rounded-2xl border ${cardBg} shadow-md`}>
        ...
        <section className={`px-6 py-24 ${altBg} sm:py-32`}>
          <div className="mx-auto max-w-6xl">
            ...
            <ul className="grid grid-cols-2 gap-6 sm:grid-cols-3 lg:grid-cols-4">
              {items.map((item, i) => (
                <li key={i} className={`group relative overflow-hidden rounded-2xl border ${cardBg} shadow-md transition-shadow hover:shadow-xl`}>
                  ...
                  <div className="p-4">
                    ...
                    <div className="mt-2 flex items-center justify-between">
                      <span className="font-extrabold" style={{ color: PRIMARY }}>{item.price}</span>
                    </div>
                    <AddToCart />
                  </div>
                </li>
