        ...
        <section className={`px-6 py-24 ${altBg} sm:py-32`}>
          <div className="mx-auto max-w-5xl">
            ...
            <ul className="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-4">
              {items.map((item, i) => (
                <li key={i}>
                  <Card className="flex h-full flex-col items-center gap-3 p-6 shadow-sm">
            ...
            <div className="flex flex-wrap justify-center gap-3">
              {items.map((item, i) => (
                <Badge
                  key={i}
                  variant="outline"
                  className={`inline-flex items-center gap-2 rounded-full px-4 py-2 text-sm font-semibold shadow-sm ${textPrimary}`}
                >
                  {item.icon && <span aria-hidden="true">{item.icon}</span>}
                  {item.name}
                </Badge>
